...
export class StorybookGenerator {
  constructor(ast, componentDir) {
    ...
    this.firstVariantLayers = Object.values(ast.variantStyles)[0]?.base?.layers || [];
  }

  generate() {
    Logger.info(`[StorybookGenerator] Generating initial Story for ${this.componentName}...`);

    // Check if already exists. If yes - skip generation
    const filePath = this._getFilePath();
    if (fs.existsSync(filePath)) {
      Logger.note(`[StorybookGenerator] Skip: ${path.basename(filePath)} exists.`);
      return;
    }
    ...
    const storybookCode = storybookTemplate({
      ...
      booleanDefaultArgs: this._buildBooleanDefaultArgs(this.ast.allBooleanPropNames)
    });
 ...
 // ArgType Builders
  ...
  _buildTextArgTypes(array) {
    return mapJoin(array, p => 
      `    ${cleanPropName(p)}: { control: 'text' },`
    );
  }

  _buildBooleanArgTypes(array) {
    return mapJoin(array, p => 
      `    ${cleanPropName(p)}: { control: 'boolean' },`
    );
  }
  ...
  _buildVariantDefaultArgs(array) {
    return mapJoin(array, p => 
      `    ${cleanPropName(p.name)}: '${p.values[0]}',`
    );
  }
